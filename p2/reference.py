...
class MNISTdataset:
    def __init__(self, datapath, train: bool):
        # self.files =
...
def train_mnist():
    ...
    save_model = True
    save_dir = "./test4/"
    ws_test = [0.0, 1.0, 2.0]  # strength of generative guidance
    ...
    dataset = MNISTdataset("../hw2_data/digits/mnistm/", train=True)
...
def valid_mnist(outpath, model):
    ...
    total = 0
    with torch.no_grad():
        for inf_idx in range(10):
            n_sample = n_classes * 10
            for w_i, w in enumerate(ws_test):
                x_gen, x_gen_store = ddpm.sample(
                    n_sample, (3, 28, 28), device, guide_w=w
                )
...
if __name__ == "__main__":
    ...
    if args.train:
        ...
        valid_mnist(args.outpath, args.model)
